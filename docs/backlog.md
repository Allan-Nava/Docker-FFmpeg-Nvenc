...
- [ ] check the actual tags on GHCR afterwards: `latest`, `2.x.y`, `2.x`, `2` **without** suffix (default
      ...
      `latest-ffmpeg9.0.1`) and the exact `2.x.y-ffmpeg9.0.1`. The tag list is generated by
      `docs/scripts/image-tags.py`; the point of checking is that the registry agrees with it
